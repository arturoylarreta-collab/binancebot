...
# Client-id prefixes generated by this bot. Orders without them (e.g. placed by
# hand on the testnet UI) are never reported nor touched.
BOT_ORDER_PREFIXES = ("ENTRY-", "SL-", "TP-", "CLOSE-", "FLAT-")
...
class ReconciliationEngine:
    ...
    async def _reconcile_positions(
        ...
        issues: list = []
        # Aggregate internal exposure per symbol (net, signed) so two internal
        # positions on one symbol never look like a mismatch.
        by_symbol: dict = {}
        for pos in position_manager.open_positions():
            sign = 1.0 if pos.side == "BUY" else -1.0
            by_symbol.setdefault(pos.symbol, []).append(sign * pos.quantity)
        seen: set = set()
        for pos in position_manager.open_positions():
            if pos.symbol in seen:
                continue
            seen.add(pos.symbol)
            net = sum(by_symbol[pos.symbol])
            if abs(net) < 1e-12:
                continue
            ...
            same_side = (venue_side == "LONG") == (net > 0)
            if not same_side or not _qty_close(venue_qty, abs(net)):
                issues.append(ReconciliationIssue(
                    "QTY_MISMATCH", pos.symbol,
                    f"internal net {net} vs venue {venue_side} {venue_qty}",
                ))
    ...
    async def _reconcile_orphans(
        ...
        # Only protections of positions that are still open count as tracked:
        # a SL/TP left resting after its position closed IS an orphan.
        tracked: set = set()
        for pos in position_manager.open_positions():
        ...
        for report in await adapter.open_orders():
            if not report.client_order_id.startswith(BOT_ORDER_PREFIXES):
                continue  # manual / foreign order on the account: not ours
